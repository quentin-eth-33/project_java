...
public class UserInterface {
    ...
    public void startGame() {
        ...
        while (true) {
            ...
            System.out.println("3. Supprimer un bâtiment");
            System.out.println("4. Ajouter des travailleurs / habitants à un bâtiment");
            System.out.println("5. Supprimer des travailleurs / habitants d'un bâtiment");
            System.out.println("6. Faire evoluer un batiment");
    ...
    private void upgradeBuilding() {
        ...
        if (buildingChoice >= 1 && buildingChoice <= gameManager.getNbBuildingType()) {
            ...
            if(nbBuildingAvailable(buildings) >0){
                ...
                System.out.println("Aucun batiment n'est disponible");
            }
    ...
    private void addPeople() {
        ...
        System.out.println("Choisissez le type bâtiment où ajouter des personnes :");
        int selectedBuildingIndex = scanner.nextInt();

        if (selectedBuildingIndex >= 1 && selectedBuildingIndex <= gameManager.getNbBuildingType()) {
    ...
    private void removePeople() {
        ...
        System.out.println("Choisissez le type bâtiment où supprimer les personnes :");
        int selectedBuildingIndex = scanner.nextInt();
        if (selectedBuildingIndex >= 1 && selectedBuildingIndex <= gameManager.getNbBuildingType()) {
            BuildingType selectedBuildingType = BuildingType.values()[selectedBuildingIndex - 1];
            List<Building> buildings = gameManager.getBuildings(selectedBuildingType);
    ...
    private void removeBuilding() {
        ...
        System.out.println("Choisissez le type de bâtiment que vous souhaitez supprimer:");
        int selectedBuildingIndex = scanner.nextInt();
        ...
        if (selectedBuildingIndex >= 1 && selectedBuildingIndex <= gameManager.getNbBuildingType()) {
            ...
            if(nbBuildingAvailable(buildings) >0){
                System.out.println("Bâtiments disponibles pour " + selectedBuildingType.name() + " :");
                displayBuildings(buildings);
                System.out.println("gameManager.getNbBuildingType():  "+gameManager.getNbBuildingType());



                System.out.println("Entrez l'ID du bâtiment " + buildings.get(0).getType().name() + " que vous souhaitez supprimer :");
                int batID = scanner.nextInt();
                ...
                if (building != null) {
                    ...
                    actionPerfomed();
                    System.out.println("Bâtiment supprime.");
                } else {
    ...
    private void printBuildingState() {
        ...
        for (BuildingType buildingType : BuildingType.values()) {
            ...
            if (!buildings.isEmpty()) {
                ...
                for (Building building : buildings) {
                    if (building.isConstructionComplete()) {
                        System.out.println("  - ID: "+building.getId()+" | " + building.getType().name() + " | Level: "+building.getCurrentLevel()+
    ...
    private Building getBuilding(BuildingType buildingType){
        ...
        switch (buildingType) {
            ...
            case APARTMENT_BUILDING:
                buildingFactory = new ApartmentBuildingFactory();
                break;
            case FARM:
                buildingFactory = new FarmFactory();
                break;
            case QUARRY:
                buildingFactory = new QuarryFactory();
                break;
            case LUMBER_MILL:
    ...
    private int nbBuildingAvailable(List<Building> buildings){
        int cpt =0;
        for (Building building : buildings) {
            if(building.isConstructionComplete()){
                cpt++;
